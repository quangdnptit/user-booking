    ...
    <main class="flex-1 w-full mx-auto px-4 sm:px-6 py-8">
      <div class="flex flex-col xl:flex-row xl:gap-8 max-w-[1600px] mx-auto">
        ...
        <div class="flex-1 min-w-0 order-1 xl:order-2">
          <div class="mb-8">
            <h1 class="font-display font-bold text-3xl text-white">Now showing</h1>
            <p class="text-cinema-muted mt-1">Pick a movie and book your seats</p>
          </div>
          ...
          <Transition name="fade" mode="out-in">
            ...
            <div v-else key="grid" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              <router-link
                ...
                class="group block rounded-2xl border border-white/[0.06] bg-cinema-panel overflow-hidden shadow-cinema hover:border-emerald-500/30 hover:shadow-glow transition-all duration-300 ease-smooth animate-fade-in-up opacity-0"
              >
                <div class="aspect-[2/3] overflow-hidden bg-cinema-surface">
                  <img
                    :src="m.posterUrl || 'https://placehold.co/400x600/14181f/9ca3af?text=No+poster'"
                    :alt="m.title"
                ...
                <div class="p-4">
                  <h2
                    class="font-semibold text-white group-hover:text-emerald-400 transition-colors duration-200 line-clamp-2"
                  >
                    {{ m.title }}
                  </h2>
                  <p class="text-sm text-cinema-muted mt-1">{{ m.genre }} · {{ m.durationMinutes }} min</p>
                  <span
                    class="inline-block mt-2 text-sm font-medium text-emerald-400 transition-transform duration-200 group-hover:translate-x-0.5"
                    >Book tickets →</span
                  >
                </div>
              </router-link>
